...
public:
  ...
  /* Node() { }; */
  Node(const std::string &name_, pid_t pid_ = 0, pid_t ppid_= 0,
       uid_t uid_ = 0)
    : name(name_), pid(pid_), ppid(ppid_), uid(uid_), parent(NULL) { };
}; // class Node

class Tree {
public:
  std::map<pid_t,Node *> nodes;
  int rootCount;
  ...
  int build();
  void clear();
  void insertNode (const std::string &name, pid_t pid, pid_t ppid, uid_t uid);
  Node * findNodeByPID (pid_t pid);
  int countChildren (pid_t pid);
  int signalChildren (pid_t pid, int sigNum);
...
#define PROCFS_ROOT "/proc"
    ...
    if (dirEnt->d_type == DT_DIR) {
      ...
      while (fgets(linebuf, sizeof(linebuf), fp) != NULL) {
        ...
        if (haveUid && havePPid && havePid && haveName) {
          insertNode (procname, pid, ppid, uid);
  ...
  for (j = 0;  j < nJobs;  j++) {
    ...
    insertNode (procName, pid, ppid, job[j].kp_eproc.e_pcred.p_ruid);
#if 0
...
/* # error "NO IMPLEMENTATION for getting process list for Tree" */
  ...
  if (! foundInitProc) {
    insertNode ("init", (pid_t) 0, (pid_t) 0, (uid_t) 0);
  }

  Node *curr, *parent;
  // link the nodes in the list to form a tree (or a forest of trees)
  /* for (auto& cur: nodes) { */
  for (std::map<pid_t,Node *>::iterator it=nodes.begin();  it != nodes.end();  ++it) {
    curr = it->second;
...
void Tree::insertNode (const std::string &name, pid_t pid, pid_t ppid, uid_t uid)
{
  if (nodes.find(pid) != nodes.end()) /* silently ignore duplicate insertion */
    return;
  nodes[pid] = new Node(name, pid, ppid, uid);
} // insertNode

Node * Tree::findNodeByPID (pid_t pid)
{
#if 0
  try {
    return nodes.at(pid);
...
#else
  std::map<pid_t,Node *>::iterator it = nodes.find(pid);
  if (nodes.end() == it)
    return NULL;
  return it->second;
#endif
...
int Tree::countChildren (pid_t pid)
{
  Node *curr;
#if 0
  try {
...
int Tree::signalChildren (pid_t pid, int sigNum)
{
  Node *curr;
#if 0
  try {
...
#endif /* ! (defined(__sparc) || defined(_WIN32)) */
...
#endif /* #if (defined(__sparc) || defined(_WIN32)) .. else .. */
